...
> A wiki stores pages and links. A vector database stores embeddings for similarity search.
...
The threshold controls which matches you keep. Raise it to accept only closer matches; lower it to include more candidates.
...
| Problem | Wiki approach | Vector approach |
| --- | --- | --- |
...
> **The tradeoff:** The wiki approach costs more tokens but buys you structured pages and citation chains that people can read and revise. The vector approach is cheaper per query but gives you ranked chunks — no structure, no connections, no synthesis.
...
| Harness type | Medium | Token cost | What it can enforce |
| --- | --- | --- | --- |
